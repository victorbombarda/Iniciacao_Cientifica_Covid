func_grafico_CovidLP_15 <- function(pais, estado = NULL, data_base){
  ...
  iteracoes <- floor(as.numeric(as.Date("2020-12-22") - as.Date(data_base)) /15 )
  plot_list = list()
  previsoes_list = list()
  ...
  for(i in 1:iteracoes){
    ...
    df$data <-filter(df$data, df$data$date < as.Date(data_base) + i*15) 
    ...
    df2 <- filter(df2$data, df2$data$date >= as.Date(data_base) + i*15)
  ...
  for (i in 1:iteracoes) {
    file_name = paste("COVIDLP_15dias_",pais, "_", estado,"_", i, ".png", sep="")
    file_name_csv =paste("COVIDLP_15dias_",pais, "_", estado,"_", i, ".csv", sep="") 
